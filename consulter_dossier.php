...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Validation_DGAM_Dossier'])) {
    if (isset($_POST['Validation_DGAM_Dossier'])) {
        $validation = $_POST['Validation_DGAM_Dossier'];
        $commentaire = mysqli_real_escape_string($conn, $_POST['commentaire']); // Échapper les caractères spéciaux

        // Mettre à jour l'attribut Validation dans la table Dossier
        $requeteMiseAJour = $conn->prepare('UPDATE dossier SET Validation_DGAM_Dossier = ? WHERE ID_dossier = ?');
        $requeteMiseAJour->bind_param('ii', $validation, $dossierID);
        ...
        $insertSql = "INSERT INTO Valider (ID_user, ID_dossier, Date_Validation, Commentaire) VALUES ('$userID', '$dossierID', NOW(), '$commentaire')";
...
<?php include_once('header.php'); ?>
    ...
    <table class="table table-striped">
        ...
        <h3>Modifier la validation</h3>
        <form method="POST" action="">
            <div class="form-group">
                <label for="Validation_DGAM_Dossier">Validation :</label>
                <select class="form-control" name="Validation_DGAM_Dossier" id="Validation_DGAM_Dossier">
                    <option value="0" <?php echo $row['Validation_DGAM_Dossier'] == 0 ? 'selected' : ''; ?>>En attente</option>
                    <option value="1" <?php echo $row['Validation_DGAM_Dossier'] == 1 ? 'selected' : ''; ?>>Validé</option>
                    <option value="2" <?php echo $row['Validation_DGAM_Dossier'] == 2 ? 'selected' : ''; ?>>Refusé</option>
                </select>
            </div>
            <div class="form-group">
                <label for="commentaire">Commentaire :</label>
                <textarea class="form-control" name="commentaire" id="commentaire" rows="3"><?php echo $row['Commentaire']; ?></textarea>
            </div>
